...
import { MONTHS, daysInMonth, parseLocalDate } from "../dates";
...
function ExcelGridView({
  ...
  const { byRowAndCategory, rowTotals, categoryTotals } = pivot(expenses, categories, rows, mode);
  const grandTotal = Object.values(categoryTotals).reduce((a, b) => a + b, 0);
  const tableClass =
    mode === "month" ? "month-table" : mode === "week" ? "week-table" : "year-table";
  return (
      ...
      <table className={`excel-table ${tableClass}`}>
        <thead>
          <tr>
            ...
            {rows.map((row) => (
              <th key={row.key} className={row.cellClass}>
                {row.label}
              </th>
            ))}
